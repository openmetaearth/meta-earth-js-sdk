...
export const txClient = (
  ...
  return {
    ...
    async sendMsgUndelegate({ value, fee, memo, gas = 0 }: any): Promise<any> {
      if (!signer) {
    ...
    async sendMsgCancelUnbondingDelegation({
      ...
      try {
        ...
        throw new Error(
          'TxClient:sendMsgCancelUnbondingDelegation: Could not broadcast Tx: ' + e.message,
        )
      }
    },
    ...
    async sendMsgDelegate({ value, fee, memo }: any): Promise<any> {
      if (!signer) {
      ...
      try {
        ...
        const gas_fee = getFinalGas(gasUsed, 0)
        const fee = {
          amount: [{ denom: 'umec', amount: `${gas_fee}` }],
          gas: gas_max_set,
        }
    ...
    msgCancelUnbondingDelegation({ value }: msgCancelUnbondingDelegationParams): EncodeObject {
      try {
        ...
        throw new Error(
          'TxClient:MsgCancelUnbondingDelegation: Could not create message: ' + e.message,
        )
      }
    },
    ...
    async simulateSendGas({ value, gas = 50000, msg, type }: any) {
      ...
      if (msg) {
        ...
        switch (type) {
          // Flexible staking
          case 'MsgDelegate':
            _msg = this.msgDelegate({ value: MsgDelegate.fromPartial(value) })
            break
          // Withdraw flexible staking
          case 'MsgUndelegate':
            _msg = this.msgUndelegate({ value: MsgUndelegate.fromPartial(value) })
            break
          default:
            break
        }
