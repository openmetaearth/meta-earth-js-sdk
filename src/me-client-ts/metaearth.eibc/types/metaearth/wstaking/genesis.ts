...
export const GenesisState: MessageFns<GenesisState> = {
  encode(message: GenesisState, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    for (const v of message.stakes) {
      Stake.encode(v!, writer.uint32(66).fork()).join()
    }
    for (const v of message.unbondingStakes) {
      UnbondingStake.encode(v!, writer.uint32(74).fork()).join()
    }
    for (const v of message.regions) {
      Region.encode(v!, writer.uint32(82).fork()).join()
    }
    for (const v of message.fixedDepositList) {
      FixedDeposit.encode(v!, writer.uint32(90).fork()).join()
    }
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 12: {
          ...
          message.fixedDepositCount = longToNumber(reader.uint64())
          continue
        }
        case 13: {
          if (tag !== 104) {
  ...
  fromJSON(object: any): GenesisState {
    return {
      ...
      stakes: globalThis.Array.isArray(object?.stakes)
        ...
        : [],
      unbondingStakes: globalThis.Array.isArray(object?.unbondingStakes)
        ? object.unbondingStakes.map((e: any) => UnbondingStake.fromJSON(e))
        : globalThis.Array.isArray(object?.unbonding_stakes)
          ? object.unbonding_stakes.map((e: any) => UnbondingStake.fromJSON(e))
          : [],
      regions: globalThis.Array.isArray(object?.regions)
        ? object.regions.map((e: any) => Region.fromJSON(e))
        : [],
      fixedDepositList: globalThis.Array.isArray(object?.fixedDepositList)
        ? object.fixedDepositList.map((e: any) => FixedDeposit.fromJSON(e))
        : [],
      fixedDepositCount: isSet(object.fixedDepositCount)
        ? globalThis.Number(object.fixedDepositCount)
        : 0,
  ...
  toJSON(message: GenesisState): unknown {
    ...
    if (message.unbondingStakes?.length) {
      obj.unbondingStakes = message.unbondingStakes.map((e) => UnbondingStake.toJSON(e))
    }
    if (message.regions?.length) {
      obj.regions = message.regions.map((e) => Region.toJSON(e))
    }
    if (message.fixedDepositList?.length) {
      obj.fixedDepositList = message.fixedDepositList.map((e) => FixedDeposit.toJSON(e))
    }
    if (message.fixedDepositCount !== 0) {
