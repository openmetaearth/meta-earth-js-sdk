...
export const txClient = (
  ...
  return {
    async sendMsgPayPacketFee({
      value,
      fee,
      memo,
    }: sendMsgPayPacketFeeParams): Promise<DeliverTxResponse> {
      if (!signer) {
    ...
    async sendMsgPayPacketFeeAsync({
      value,
      fee,
      memo,
    }: sendMsgPayPacketFeeAsyncParams): Promise<DeliverTxResponse> {
      if (!signer) {
        throw new Error(
          'TxClient:sendMsgPayPacketFeeAsync: Unable to sign Tx. Signer is not present.',
        )
      }
      try {
        const { address } = (await signer.getAccounts())[0]
